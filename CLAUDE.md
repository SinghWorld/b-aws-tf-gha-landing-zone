...
- **Destroy workflow leaves OIDC out of state**: `.github/workflows/destroy.yml` `terraform state rm`s the four `module.github_oidc.*` resources before generating the destroy plan, so the OIDC role and provider survive in AWS but are no longer tracked by Terraform. This is required because Terraform has no `-exclude` flag and `lifecycle.prevent_destroy` aborts the entire destroy. After a CI destroy, to use the plan-apply workflow again you must either re-import the OIDC resources (see import commands in `modules/github-oidc/main.tf`) or run `scripts/01.setup_s3-backend.sh` + a local `terraform apply` to re-bootstrap from scratch.
